...
const (
	qulTafsirBaseURL = "https://qul.tarteel.ai/resources/tafsir"
	fileMode         = 0o644
)

var errMissingTafsir = errors.New("missing tafsir text")

var tafsirs = []TafsirResource{
	{
...
func (r TafsirResource) Source() string {
	return fmt.Sprintf("%s/%d", qulTafsirBaseURL, r.ID)
}
...
func main() {
	var (
		...
		only         = flag.String("only", "", "comma-separated QUL resource IDs to scrape; defaults to all configured tafsirs")
	...
	selected, err := selectTafsirs(*only)
	if err != nil {
		log.Fatal(err)
	}

	for _, tafsir := range selected {
		log.Printf("scraping %s (%d)", tafsir.Name, tafsir.ID)

		ayahs, emptyAyahs, err := scrapeAll(client, tafsir, surahs, *workers, *delay)
		if err != nil {
			log.Fatal(err)
		}

		outputDir := filepath.Join("tafsir", tafsir.Slug)
		if err := writeAyahs(outputDir, ayahs, emptyAyahs); err != nil {
			log.Fatal(err)
		}
		if err := addEdition("data/editions.json", tafsir); err != nil {
			log.Fatal(err)
		}
		if err := addEdition("tafsir/editions.json", tafsir); err != nil {
			log.Fatal(err)
		}
	}
}

func selectTafsirs(only string) ([]TafsirResource, error) {
	...
	var selected []TafsirResource
	for _, tafsir := range tafsirs {
		id := fmt.Sprintf("%d", tafsir.ID)
		if _, ok := requested[id]; ok {
			requested[id] = true
			selected = append(selected, tafsir)
		}
	}

	for id, found := range requested {
		if !found {
			return nil, fmt.Errorf("unknown configured tafsir resource ID: %s", id)
		}
	}
...
func scrapeAll(client *http.Client, tafsir TafsirResource, surahs []SurahInfo, workers int, delay time.Duration) ([]Ayah, []EmptyAyah, error) {
	if workers < 1 {
		workers = 1
	}
	...
	for i := 0; i < workers; i++ {
		...
		go func() {
			...
			for item := range jobs {
				...
				ayah, err := scrapeAyah(client, tafsir, item.Surah, item.Ayah)
				if err != nil {
					select {
					case errs <- err:
					default:
	...
	for item := range results {
		...
		if item.Ayah.Text == "" {
			...
			log.Printf("%s: missing tafsir for %d:%d", tafsir.Slug, item.Empty.Surah, item.Empty.Ayah)
		} else {
			ayahs = append(ayahs, item.Ayah)
		}
		if seen%100 == 0 {
			log.Printf("%s: processed %d ayahs", tafsir.Slug, seen)
		}
	}
...
func scrapeAyah(client *http.Client, tafsir TafsirResource, surah, ayah int) (result, error) {
	url := fmt.Sprintf("%s?ayah=%d%%3A%d", tafsir.Source(), surah, ayah)
	var lastErr error
...
func fetchTafsirText(client *http.Client, url string) (string, error) {
	...
	req.Header.Set("User-Agent", "tafsir-api-qul-import/1.0")
...
func firstNonEmptySelection(doc *goquery.Document, selectors ...string) *goquery.Selection {
	for _, selector := range selectors {
		...
		if node.Length() != 0 && strings.TrimSpace(node.Text()) != "" {
			return node
		}
	}
...
func addEdition(path string, tafsir TafsirResource) error {
	data, err := os.ReadFile(path)
	if err != nil {
		return err
	}
